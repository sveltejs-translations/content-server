...
Компонент, определяющий страницу или макет, может экспортировать функцию `load`, которая запускается до создания компонента. Эта функция выполняется как во время рендеринга на стороне сервера, так и в клиенте и позволяет извлекать данные и манипулировать ими до рендеринга страницы, тем самым предотвращая загрузку счетчиков.

Если данные для страницы поступают с ее конечной точки, вам может не понадобиться функция `load`. Это полезно, когда вам нужна большая гибкость, например, загрузка данных из внешнего API.
...
Если страница, которую вы загружаете, имеет эндпоинт, данные, возвращаемые из него, доступны внутри  `load` как `props`. Для компонентов макета и страниц без эндпоинтов `props` будет пустым объектом.
