...
Этот параметр также влияет на [пререндеринг](/docs#parametry-straniczy-prerender). Если `trailingSlash` является `always`, такой маршрут, как `/about`, приведет к созданию файла `about/index.html`, в противном случае он создаст `about.html`, отражая статические соглашения веб-сервера.
