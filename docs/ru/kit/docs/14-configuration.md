...
- `name` - текущая строка версии приложения
- `pollInterval` - интервал в миллисекундах для опроса для изменения версии

Навигация на стороне клиента может быть глючной, если вы развертываете новую версию своего приложения, пока люди используют ее. Если код для новой страницы уже загружен, он может иметь устаревший контент; если это не так, манифест маршрута приложения может указывать на файл JavaScript, который больше не существует. SvelteKit решает эту проблему, возвращаясь к традиционной полностраничной навигации, если обнаружит, что была развернута новая версия, используя указанное здесь `name` (по умолчанию используется метка времени сборки).

Если вы установите ненулевое значение `pollInterval`, SvelteKit опросит новые версии в фоновом режиме и установит значение хранилища [`updated`](#moduli-$app-stores) в `true`, когда оно его обнаружит.
