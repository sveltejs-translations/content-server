...
Это вызвало проблемы и больше не так в SvelteKit. Вместо этого URL-адреса разрешаются по текущей странице (или целевой странице для URL-адресов `fetch` в функциях `load`).
