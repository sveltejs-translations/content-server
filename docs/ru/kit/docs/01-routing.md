...
В редких случаях приведенный выше порядок может быть не темБ что нужно для данного пути. Например, возможно, `/foo-abc` должен разрешиться в `src/routes/foo-[bar].svelte`, но `/foo-def` должен разрешаться в `src/routes/[b].svelte`.
