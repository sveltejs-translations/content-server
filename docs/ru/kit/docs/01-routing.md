...
Если вы запросите маршрут с заголовком `accept: application/json`, SvelteKit отобразит данные эндпоинта в формате JSON, а не страницу в формате HTML. Вы также можете получить необработанные данные, добавив `/__data.json` к URL, например `/items/__data.json`.
