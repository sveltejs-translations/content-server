...
Когда вы переходите по адресу `/foo-xyz`, SvelteKit сначала попробует `foo-[bar].svelte`, потому что это лучшее совпадение. Если страница не даст ответа, SvelteKit попробует другие менее конкретные, но все еще действительные совпадения для `/foo-xyz`. Поскольку эндпоинты имеют более высокий приоритет, чем страницы, следующей попыткой будет `[baz].js`. Затем приоритетным будет алфавитный порядок, поэтому следующими будут спрошены `[baz].svelte`, затем `[qux].svelte`. Будет использован первый маршрут, который вернет какой-либо ответ, т.е. либо это страница, которая возвращает что-то из [`load`](#zagruzka-dannyh) функции или не имеет её вовсе, либо эндпоинт, который что-то возвращает в ответе.
