...
- `getStores` – это удобная функция-оболочка вокруг `getContext`, которая возвращает `{ navigating, page, session, updated }`. Это необходимо вызывать на верхнем уровне или синхронно во время инициализации компонента или страницы.
...
- `updated` - это [читаемое хранилище](https://svelte.dev/tutorial/readable-stores), начальное значение которого равно false. Если [`version.pollInterval`](#konfiguracziya-version) с ненулевым значением, SvelteKit опросит новые версии приложения и обновит значение хранилища до `true`, когда обнаружит его. `updated.check()` принудительно проверит, независимо от опроса.
