...
[компонентов на стороне сервера](/docs#rantajm-api-komponenta-na-servere).
...
Связывает произвольный объект `context` с текущим компонентом и указанным ключом `key`и возвращает объект. После этого, при помощи метода `getContext`, контекст становится доступным для всех дочерних элементов компонента (включая содержимое слотов).
...
Модуль `svelte/store` предоставляет функции [readable](/docs##rantajm-svelte-store-readable), [writable](/docs##rantajm-svelte-store-writable) и [derived](/docs##rantajm-svelte-store-derived) для создания соответствующих хранилищ.

Вы _не обязаны_ использовать только эти функции для того, чтобы иметь возможность работать с [префиксом $ для хранилищ](/docs#format-komponenta-script-4-dobavte-prefiks-$-k-hranilishhu-dlya-polucheniya-ego-znacheniya) в компонентах. Любой объект, в котором есть методы `.subscribe`, `.unsubscribe` и `.set`(опционально), является валидным хранилищем и будет работать как с соответствующим синтаксисом в компонентах, так и с [`производными` хранилищами](/docs##rantajm-svelte-store-derived).

Таким образом можно легко создать обертку практически для любой библиотеки управления состоянием для использования её в Svelte. Прочитайте про [контракт хранилища](/docs#format-komponenta-script-4-dobavte-prefiks-$-k-hranilishhu-dlya-polucheniya-ego-znacheniya-store-contract) для правильной реализации подобной обертки.
...
- `easing` (`function`, по умолчанию `t => t`) — [функция плавности](/docs#rantajm-svelte-easing)
...
Модуль `svelte / transition` экспортирует семь функций:`fade`, `blur`, `fly`,`slide`, `scale`,`draw` и `crossfade`. Они предназначены для использования в [`переходах`](/docs#sintaksis-shablonov-direktivy-elementov-transition-fn).
...
- `easing` (`function`, по умолчанию `linear`) — [функция плавности](/docs#rantajm-svelte-easing)
...
- `duration` (`number`, по умолчанию 400) — длительность перехода в миллисекундах
- `easing` (`function`, по умолчанию `cubicOut`) — [функция плавности](/docs#rantajm-svelte-easing)
...
- `easing` (`function`, по умолчанию `cubicInOut`) — [функция плавности](/docs#rantajm-svelte-easing)
...
- `easing` (`function`, default `cubicOut`) — [функция плавности](/docs#rantajm-svelte-easing)
- `fallback` (`function`) — Откат [transition](/docs#sintaksis-shablonov-direktivy-elementov-transition-fn), используемый для отправки, когда не получен соответствующий элемент, и для получения, когда нет отправляемого элемента.
...
Модуль `svelte/animate` экспортирует единственную функцию, которая используется для отображения [анимации](/docs#sintaksis-shablonov-direktivy-elementov-animate-fn).
...
Параметр `hydrate` инструктирует Svelte обновить существующий DOM (обычно полученный в ходе рендеринга на стороне сервера), а не создавать новые элементы. Это будет работать только в том случае, если компонент был скомпилирован с параметром [`hydratable: true`](/docs##rantajm-svelte-store). Гидратация элемента `<head>` работает корректно лишь в случае, когда код на стороне сервера также был скомпилирован с параметром `hydratable: true`, который добавляет маркер каждому элементу внутри `<head>`, благодаря которому которому компонент знает какие их элементов может удалить в процессе гидратации.
...
Компоненты Svelte также могут быть скомпилированы в пользовательские элементы (или web-компоненты) с помощью параметра компилятора `customElement: true`. Вы должны указать имя тега для компонента, используя [элемент](/docs#sintaksis-shablonov-svelte-options) `<svelte:options>`.
...
По умолчанию пользовательские элементы компилируются с параметром `accessors: true`, то есть любые [свойства](/docs#sintaksis-shablonov-atributy-i-svojstva) будут представлены как свойства DOM-элемента (а также, при возможности, будут доступны для чтения и записи как атрибуты).
...
Вы можете импортировать компонент Svelte непосредственно в Node с помощью [`svelte/register`](/docs#rantajm-svelte-register).
