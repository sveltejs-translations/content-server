...
Svelte использует ключевое слово `export`, чтобы пометить объявление переменной как _свойство_, что означает, что оно становится доступным извне всему, что будет использовать этот компонент (см. раздел [Атрибуты и свойства](/docs#sintaksis-shablonov-atributy-i-svojstva) для дополнительной информации).
...
Доступ к переменным только для чтения можно получить в качестве свойств элемента, привязанных к компоненту с помощью синтаксиса [`bind:this`](/docs#sintaksis-shablonov-direktivy-elementov-bind-this).
...
[Модуль `svelte/store`](/docs##rantajm-svelte-store) содержит минимальную реализацию хранилищ, соответствующих такому контракту.
...
Вы можете создавать собственные варианты хранилищ, без участия [`svelte/store`](/docs##rantajm-svelte-store), реализовав _контракт хранилища_:
...
> [хранилища](/docs##rantajm-svelte-store).
