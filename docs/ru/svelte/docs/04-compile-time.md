...
Как правило, вы не будете взаимодействовать с компилятором Svelte напрямую, а вместо этого интегрируете его в свою систему сборки с помощью подключаемого модуля. Команда Svelte больше всего рекомендует и инвестирует в плагин для сборки, — [vite-plugin-svelte](https://github.com/sveltejs/vite-plugin-svelte). Фреймворк [SvelteKit](https://ru.kit.svelte.dev/) использует `vite-plugin-svelte` для создания приложений, а также [инструмент для упаковки библиотек компонентов Svelte](https://ru.kit.svelte.dev/docs#sozdanie-paketov). Svelte Society поддерживает список [других подключаемых модулей](https://sveltesociety.dev/tools/#bundling) для дополнительных инструментов, таких как Rollup и Webpack.
...
| `css` | `true` | Если `true`, стили будут включены в класс JavaScript и внедрены во время выполнения для фактически визуализируемых компонентов. Если `false`, CSS будет возвращен в поле `css` результата компиляции. Большинство подключаемых модулей Svelte устанавливают для этого параметра значение `false` и используют статически сгенерированный CSS для повышения производительности, поскольку это приводит к уменьшению размера пакетов JavaScript, а выходные данные могут быть представлены в виде кэшируемых файлов `.css`.
