...
Если вы используете директивы `bind:` и `on:` вместе, то порядок, в котором они определены, влияет на значение связанной переменной при вызове обработчика событий.
...
Здесь мы привязывались к значению ввода текста, который использует событие `input`. Привязки к другим элементам могут использовать различные события, такие как `change`.
